    ...
    workflow = {
        ...
        initLinks_Tables_Fields: function (databases) {
            ...
            for (var i = 0; i < databases.length; i++) {
                var fields = workflow.getFields(databases[i].table);
                for (var k = 0; k < fields.length; k++) {
                    workflow. links_tables_fields.push({ link: '', linkName:'', table:'', field: fields[k].f_name, fieldNote: fields[k].value });
                }
            }
        },
        //添加步骤
        ...
        flowAttrSetting: function (isAdd) {
            ...
            $.ajaxSettings.async = false;
common.post("/api/workflow/GetJSON?flowid=" + id,{},function(json){
...
$.ajaxSettings.async = true;
        ...
        saveFlow: function (op) {
            ...
            if (op == "delete") {
                ...
                var json = JSON.stringify(workflow.wf_json);$.ajaxSettings.async = false;
